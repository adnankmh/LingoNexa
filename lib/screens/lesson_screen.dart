...
class _LessonScreenState extends State<LessonScreen> {
                ...
                children: [
                  ...
                  if (_checked) ...[
                    Container(
                      ...
                      decoration: BoxDecoration(
                        color: (_correct ? Colors.green : Colors.orange).withValues(alpha: .12),
                  ...
                  FilledButton(
                    ...
                    child: Row(
                      ...
                      children: [
                        Text(_checked ? context.text.get('next') : context.text.get('check')),
  ...
  bool get _canCheck {
    ...
    if (_step.type == ExerciseType.speaking) return _recognizedSpeech.isNotEmpty || _revealed;
    if (_step.type == ExerciseType.fillBlank) return _answerController.text.trim().isNotEmpty;
  ...
  void _check() {
    ...
    final submitted = switch (_step.type) {
      ...
      ExerciseType.speaking => _recognizedSpeech.isEmpty ? expected : _normalize(_recognizedSpeech),
      ExerciseType.fillBlank => _normalize(_answerController.text),
      ExerciseType.choice || ExerciseType.listening => _normalize(_selectedAnswer ?? ''),
    };
    setState(() {
      _checked = true;
  ...
  Future<void> _toggleListening() async {
    ...
    if (!started && mounted) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Speech recognition is unavailable on this device. Practice mode enabled.')));
    }
  }
  ...
  double _similarity(String a, String b) {
    ...
    return aWords.intersection(bWords).length / math.max(aWords.length, bWords.length);
  }
    ...
    return Column(
      ...
      children: [
        ...
        if (step.hint.isNotEmpty) ...[
          const SizedBox(height: 8),
          Text(step.hint, style: TextStyle(color: Theme.of(context).colorScheme.onSurfaceVariant, height: 1.45)),
        ],
    ...
    switch (step.type) {
          ...
          children: [
            ...
            for (final option in step.options)
              Padding(
                ...
                child: _AnswerOption(text: option, selected: selectedAnswer == option, onTap: () => onSelect(option)),
              ),
          ],
        );
        ...
        return Column(
          children: [
            Container(
              ...
              child: Wrap(spacing: 8, runSpacing: 8, children: [for (final word in arranged) InputChip(label: Text(word), onDeleted: () => onRemoveArranged(word))]),
            ),
            const SizedBox(height: 18),
            Wrap(spacing: 9, runSpacing: 9, children: [for (final word in step.options) if (!arranged.contains(word)) ActionChip(label: Text(word), onPressed: () => onArrange(word))]),
          ],
        );
      case ExerciseType.speaking:
        return Center(
          child: Column(
            children: [
              Text(step.answer, textAlign: TextAlign.center, style: const TextStyle(fontWeight: FontWeight.w900, fontSize: 28)),
              if (step.translation.isNotEmpty) Text(step.translation, style: TextStyle(color: Theme.of(context).colorScheme.onSurfaceVariant)),
              ...
              GestureDetector(
                ...
                child: AnimatedContainer(
                  ...
                  decoration: BoxDecoration(color: Theme.of(context).colorScheme.primary, shape: BoxShape.circle, boxShadow: [BoxShadow(color: Theme.of(context).colorScheme.primary.withValues(alpha: .35), blurRadius: listening ? 32 : 18, spreadRadius: listening ? 8 : 2)]),
                  child: Icon(listening ? Icons.stop_rounded : Icons.mic_rounded, color: Theme.of(context).colorScheme.onPrimary, size: 40),
                ),
              ),
              const SizedBox(height: 18),
              Text(recognizedSpeech.isEmpty ? 'Tap and speak' : recognizedSpeech, textAlign: TextAlign.center, style: const TextStyle(fontWeight: FontWeight.w700)),
              const SizedBox(height: 10),
              TextButton(onPressed: onReveal, child: const Text('Microphone unavailable? Continue in practice mode')),
            ],
          ),
        );
      case ExerciseType.fillBlank:
        return TextField(
          ...
          decoration: const InputDecoration(prefixIcon: Icon(Icons.edit_rounded), hintText: 'Type your answer'),
        );
      case ExerciseType.culture:
        return Container(
          width: double.infinity,
          padding: const EdgeInsets.all(24),
          decoration: BoxDecoration(color: Theme.of(context).colorScheme.tertiaryContainer, borderRadius: BorderRadius.circular(26)),
          child: Column(children: [const Text('🌍', style: TextStyle(fontSize: 54)), const SizedBox(height: 12), Text(step.translation, textAlign: TextAlign.center, style: const TextStyle(fontWeight: FontWeight.w700, height: 1.55, fontSize: 16))]),
        );
    }
  }
...
class _AnswerOption extends StatelessWidget {
  const _AnswerOption({required this.text, required this.selected, required this.onTap});
  ...
  Widget build(BuildContext context) {
    ...
    return Material(
      ...
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(19), side: BorderSide(color: selected ? scheme.primary : Theme.of(context).dividerColor, width: selected ? 2 : 1)),
      child: InkWell(
        onTap: onTap,
        borderRadius: BorderRadius.circular(19),
        child: Padding(
          padding: const EdgeInsets.symmetric(horizontal: 17, vertical: 16),
          child: Row(children: [Expanded(child: Text(text, style: const TextStyle(fontWeight: FontWeight.w800, fontSize: 16))), if (selected) Icon(Icons.check_circle_rounded, color: scheme.primary)]),
        ),
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    return InkWell(
      ...
      child: Container(
        ...
        child: Icon(Icons.volume_up_rounded, color: Theme.of(context).colorScheme.primary, size: 43),
      ),
    );
  }
}

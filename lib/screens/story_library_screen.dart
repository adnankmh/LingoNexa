...
class _StoryLibraryScreenState extends State<StoryLibraryScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final stories =
        _storiesFor(language.code, _level, state.locale.languageCode);
    return Scaffold(
      appBar: AppBar(title: const Text('Verified Dialogue Stories'), actions: [
        Padding(
            ...
            child: Column(children: [
              Container(
                ...
                child: Row(children: [
                  ...
                  Expanded(
                      child: Text(
                          'Every dialogue line is taken from the same verified phrase record as its meaning and target-language audio. No automatic prose translation is invented.',
                          style: TextStyle(
                              color: Theme.of(context)
                                  .colorScheme
                                  .onSurfaceVariant,
                    ...
                    children: [
                      for (final level in CourseRepository.levels)
                        Padding(
                            ...
                            child: ChoiceChip(
                                selected: _level == level.code,
                                onSelected: (_) =>
                                    setState(() => _level = level.code),
                                ...
                                child: Row(children: [
                                  ...
                                  Expanded(
                                      child: Column(
                                        ...
                                        Text(
                                            '${story.lines.length} verified lines · $_level',
                                            style: TextStyle(
                                                color: Theme.of(context)
                                                    .colorScheme
                                                    .onSurfaceVariant,
  ...
  List<_DialogueStory> _storiesFor(
      String languageCode, String level, String sourceLanguageCode) {
    final phrases = LearningContentRepository.phrasesFor(languageCode,
    ...
    final levelIndex =
        CourseRepository.levels.indexWhere((item) => item.code == level);
    return [
        ...
        _DialogueStory(
          ...
          lines: [
            for (var lineIndex = 0; lineIndex < 6; lineIndex++)
              phrases[(levelIndex * 7 + storyIndex * 3 + lineIndex) %
                  phrases.length],
          ],
        ),
    ];
...
class _StoryReaderState extends State<_StoryReader> {
  ...
  Widget build(BuildContext context) => Scaffold(
        appBar: AppBar(title: Text(widget.story.title), actions: [
          Padding(
              padding: const EdgeInsetsDirectional.only(end: 54),
              child: Center(
  ...
  Future<void> _play(String text) async {
    final spoken = await _speech.speak(text, widget.language.code,
        rate: AppStateScope.of(context).speechRate);
    if (!spoken && mounted) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content:
              Text('${widget.language.englishName} voice is not installed.')));
    }
  }
}

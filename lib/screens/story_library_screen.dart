...
class _StoryLibraryScreenState extends State<StoryLibraryScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Verified Dialogue Stories'),
        actions: [
          Padding(
  ...
  List<_DialogueStory> _storiesFor(
    ...
    const titles = [
      ...
      ('Learning together', '🗣️', 'Essentials'),
      ('At the station', '🚆', 'Travel'),
      ('The airport journey', '🛫', 'Travel'),
      ('A quick meal', '🍽️', 'Food'),
      ('Dinner with friends', '🥗', 'Food'),
      ('Checking in', '🏨', 'Hotel'),
      ('Shopping for a gift', '🎁', 'Shopping'),
      ('A busy workday', '💼', 'Work'),
      ...
      for (var storyIndex = 0; storyIndex < titles.length; storyIndex++)
        _DialogueStory(
          ...
          lines: _storyLines(
            phrases,
            titles[storyIndex].$3,
            levelIndex * 11 + storyIndex * 5,
          ),
        ),
    ];
  }

  List<PhraseEntry> _storyLines(

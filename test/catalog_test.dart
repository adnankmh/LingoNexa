...
void main() {
  test('catalog includes more than fifty distinct languages', () {
    ...
    expect(LanguageCatalog.all.map((item) => item.code).toSet().length,
        LanguageCatalog.all.length);
  });

  test('every catalog language has a starter lexicon', () {
    for (final language in LanguageCatalog.all) {
      expect(CourseRepository.starterLexicon[language.code], isNotNull,
          reason: language.code);
      expect(CourseRepository.starterLexicon[language.code], hasLength(4),
          reason: language.code);
    }
  });
  ...
  test('global content and interface localization are substantially expanded',
    ...
    expect(GlobalContentRepository.localizedPhrasePairs,
        greaterThanOrEqualTo(420));
    expect(
        GlobalContentRepository.sentenceDrillCount, greaterThanOrEqualTo(1680));
    for (final code in GlobalContentRepository.coreLanguageCodes) {
      expect(GlobalContentRepository.phrasesFor(code),
          hasLength(GlobalContentRepository.concepts.length));
      expect(GlobalContentRepository.sentenceDrillsFor(code),
          hasLength(GlobalContentRepository.concepts.length * 4));
    }
    for (final concept in GlobalContentRepository.concepts) {
      expect(concept.translations.keys,
  ...
  test('expanded learning studio covers goals, grammar, scripts, and phrases',
    ...
    expect(LearningContentRepository.alphabetSamples.length,
  ...
  test(
    ...
    for (final language in LanguageCatalog.all) {
      expect(SpeechService.voiceLocale(language.code), isNot(language.code),
  ...
  test('non-core language content never falls back to English text', () {
    ...
    expect(welsh.take(4).map((item) => item.target),
  ...
  test(
    ...
    for (final code in ['ar', 'es', 'de', 'ja', 'cy', 'sw']) {
      ...
      for (final lesson
          in CourseRepository.unitsFor(code).expand((unit) => unit.lessons)) {
        final flashcard = lesson.steps.first;
        expect(aligned[flashcard.prompt], flashcard.answer,
  ...
  test(
      ...
      () {
    final spanishForArabicUi =
        GlobalContentRepository.phrasesFor('es', sourceLanguageCode: 'ar');
    expect(spanishForArabicUi.first.target, '¿Cómo estás?');
    expect(spanishForArabicUi.first.source, 'كيف حالك؟');
    final completeSpanishForArabicUi =
        LearningContentRepository.phrasesFor('es', sourceLanguageCode: 'ar');
    expect(completeSpanishForArabicUi.any((item) => item.source == 'Hello'),
        isFalse);
    final arabicCourseForSpanishUi =
        CourseRepository.unitsFor('ar', meaningLanguageCode: 'es');
    expect(arabicCourseForSpanishUi.first.lessons.first.steps.first.prompt,
        'مرحبًا');
    expect(arabicCourseForSpanishUi.first.lessons.first.steps.first.answer,
        'Hola');
  });
}
